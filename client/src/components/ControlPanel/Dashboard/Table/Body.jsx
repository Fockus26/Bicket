...
function Body(props) {
  const {
    ...
    selectedRows,
    setSelectedRows,
    setIsAllActualRowsSelected,
    actualData,
  } = props;

  const handleDeleteRow = (id) =>
    setDataDashboard(dataDashboard.filter((value) => value.id !== id));
  ...
  const selectRow = (id) =>
    setSelectedRows((prev) => {
      ...
      const updatedSelectedRows = prev.map((item) =>
        item.id === id ? { id: item.id, isSelected: !item.isSelected } : item
      );

      // Verifica si todas las filas han sido seleccionadas
      const isAllRowsSelected = updatedSelectedRows.every(
        (item) => item.isSelected
      );
      setIsAllRowsSelected(isAllRowsSelected);

      const actualSelectedRows = updatedSelectedRows.filter((item) =>
        idActualRows.includes(item.id)
      );
      const isAllActualRowsSelected = actualSelectedRows.every(
        (item) => item.isSelected
      );
  ...
  useEffect(() => {
    const actualSelectedRows = selectedRows.filter((item) =>
      idActualRows.includes(item.id)
    );
