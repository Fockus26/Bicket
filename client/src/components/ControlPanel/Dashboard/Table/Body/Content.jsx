...
import useColorPage from "utils/useColorPage.jsx";

function Content(props) {
  const { row, dataHeader, selectedRows, selectRow, handleDeleteRow } = props;
  const navigate = useNavigate();

  const isSelected = selectedRows.find(
    (actualRow) => actualRow.id === row.id
  ).isSelected;

  const colorPage = useColorPage(row.page);

  return (
    <tr key={row.id} style={{ borderColor: colorPage.colorBase }}>
      <td style={colorPage.colorVariations}>
          ...
          {dataHeader.map((key, index) => {
            return row[key] ? (
              <div
                ...
                style={{
                  color: key === "page" ? colorPage.colorBase : undefined,
                }}
              >
                {key === "page" ? (
                  <button
        ...
        <DetailData
          colorPage={{ color: colorPage.colorBase }}
          detailData={row}
        />
      </td>
    </tr>
