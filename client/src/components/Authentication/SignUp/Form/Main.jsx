...
function MainData(props) {
  ...
  const [actualData, setActualData] = useState(
    Object.keys(mainData)[amountVerified]
  );
  ...
  const navigateData = (direction) => {
    if (direction === "prev" || mainData[actualData].isVerified) {
      const isAllVerified = Object.values(mainData).every(
  ...
  const verifyData = (key, value) => {
    switch (key) {
      ...
      case "password": {
        ...
        setShowInputRepeatPassword(
          value.length > 0 && actualData === "password"
        );
  ...
  return (
    <>
      <i
        className={
          "fa-solid fa-chevron-left " + (amountVerified === 0 ? "hide" : "")
        }
      ...
      <div className={"entries " + (isError ? "error" : "")}>
        ...
        <div className="entry">
          <input
            type={
              actualData === "password" && !showPassword ? "password" : "text"
            }
            placeholder=""
          ...
          <label className="floating">
            {isError ? errorLabel : floatingLabel}
          </label>
          <i
            className={
              !(actualData === "password")
                ? "hide"
                : "fa-solid fa-eye" + (showPassword ? "-slash" : "")
            }
        ...
        <div
          className={
            "entry " +
            (actualData === "password" && showInputRepeatPassword ? "" : "hide")
          }
          style={{
            transform:
              actualData === "password" &&
        ...
        >
          ...
          {actualData === "password" && (
            <i
              className={
                "fa-solid fa-eye" + (showRepeatPassword ? "-slash" : "")
              }
      ...
      <i
        className={
          "fa-solid fa-" +
          (actualData === "password" ? "angles" : "chevron") +
          "-right"
        }
