...
function SpecificData(props) {
  ...
  const addDropdownRef = (e) => {
    ...
    if (e && dataDropdowns.includes(e.name)) dropdownRefs.current[e.name] = e;
  };
  ...
  const renderPhone = (key) => (
    <>
      <div className="code">
        ...
        {dropdown === "code" &&
          renderDropdown(filteredPhoneCodes, (value) =>
            handleData("code", value)
          )}
      </div>
  ...
  const renderBirthday = () =>
    Object.entries(birthday).map(([key, value], index) => (
      <div key={index} onMouseLeave={() => setDropdown(null)}>
        ...
        {dropdown === key &&
          renderDropdown(filteredBirthdayDate[key], (value) =>
  ...
  const verifyValue = (key, value, isBirthday, isPhone) => {
    ...
    if (isPhone) {
      ...
      const phoneItem = phoneData.find(
        (item) => item.code === (key === "code" ? newValue : phone.code)
      );
      console.log(phoneItem);

      let isValidPhone = false;
      if (key === "code") {
        const validPhones = phoneData
          .map((item) => item.code)
          .filter((code) => code.startsWith(formattedValue.code));

        if (validPhones.length) {
          if (phoneItem && phoneItem.numberLength < phone.number.length) {
            formattedValue.number = phone.number.slice(
              0,
              phoneItem.numberLength
            );
          }
        ...
        const areDigits =
          /^\d+$/.test(formattedValue.number) || !newValue.length;
        const validLength =
          !phoneItem || phoneItem.numberLength >= formattedValue.number.length;
      ...
      if (validDate.length) {
        ...
        setFilteredBirthdayDate((prev) => {
          ...
          if (["month", "year"].includes(key)) {
            const dateItem = birthdayDate[key].find(
              (key) => key.name === newValue
            );
            ...
            if (dateItem) {
              ...
              if (key === "month") {
                ...
                if (actualDays !== dateItem.days) {
                  newBirthdayDate.day = getDate({
                    key: "day",
                    length: dateItem.days,
                  });
                }
              }
  ...
  const verifyData = (key, value, isBirthday, isPhone) => {
    if (value) {
      ...
      if (isBirthday) {
        ...
        const days = getDate({
          key: "day",
          length: monthDays ? monthDays.days : 31,
        });
      ...
      if (isPhone) {
        ...
        if (
          numberLengthItem &&
          number &&
          numberLengthItem.numberLength === number.length
        )
          return true;
      }
    }
  ...
  const handleData = (key, value) => {
    ...
    setData((prev) => {
      ...
      const amountVerified = Object.values(newSpecificData).filter(
  ...
  return (
    <>
          ...
          >
            {key === "birthday" ? (
              ...
              <>
                ...
                <div
                  className="progress"
                  style={{ width: value.isVerified && "100%" }}
                />
                {key === "country" &&
                  dropdown === "country" &&
                  renderDropdown(filteredCountries, (value) =>
                    handleData(key, value)
                  )}
              </>
            )}
      ...
      <div className="counter">
        ...
        <i
          className={
            "fa-solid fa-caret-right " + (!isAllVerified ? "hide" : "")
          }
          onClick={sendData}
        />
      </div>
