...
function Content(props) {
  ...
  const RenderPage = ({page}) => 
      <article style={useColorPage(activePage).colorVariations}>
        ...
        <div>
          {Object.entries(page).map(([featureKey, featureValue], featureIndex) => {   
            const { footer: footerFeature, header: headerFeature } = featureValue
              ...
              <section key={featureIndex}>
                ...
                <footer 
                  style={{ 
                    ...
                    margin: activeFeature === featureKey ? '0.938rem 0 1.563rem' : undefined,
                    pointerEvents: activeFeature !== featureKey ? 'none' : undefined,
                  }}
                >
  ...
  const renderMedia = mediaFeature => 
    <>
      { showVideo && 
        <div>
          <video src={mediaFeature.videoSrc} onClick={handleStartVideo} autoPlay />
          <div onClick={handleShowVideo} className="fullscreen-overlay" />  
        </div>
      }
      <img src={mediaFeature.imgSrc} alt={mediaFeature.imgAlt} onClick={handleShowVideo} />
    </>
                ...
                { activeFeature === 'searching' ? (
                  ...
                  : (
                    <div className="progress-bar">
                      { Array.from({ length: featureValue.stripes }).map((_, index) => 
