...
function Nav(props) {
  ...
  return (
    <nav>
      ...
      <ul style={{ transform: isMobile && `translateX(-${indexActivePage * 100}%)` }}>
      ...
      { isMobile && 
        <i className="fa-solid fa-angle-right" onClick={nextSlide}
          style={{
            opacity: indexActivePage === Object.keys(dataPagesNavBar).length - 1 && '0',
            pointerEvents: indexActivePage === Object.keys(dataPagesNavBar).length - 1 && 'none'
          }}
        />
      }
    </nav>
