...
function Pagination(props) {
    const { itemsPerPage, startItem, endItem, handleItemsToShow, dataDashboard, actualPage } = props
    ...
    return (
        <div className="pagination">
            <nav>
                ...
                <i 
                    className={'fa-solid fa-angle-right ' + (endItem === dataDashboard.length && 'hide')}
                    onClick={() => handlePageDirection('next')}
                />
            </nav>
            
            <span>{startItem} - {endItem} from {dataDashboard.length} tickets</span>
        </div>
    )
}
