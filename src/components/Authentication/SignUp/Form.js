...
function Form(props) {
    const { showComponent, setShowComponent, setCounterClass, setIsLogin, setStep } = props
    ...
    return (
        <form className={'container ' + (showComponent.specific ? 'specificData' : '')}>
            { showComponent.main ? (
                    <Main mainData={data.main} setData={setData} setShowComponent={setShowComponent} /> 
                ) : showComponent.specific && (
                    <Specific specificData={data.specific} setData={setData} sendData={sendData} />
                )
            }
        </form>
    )
